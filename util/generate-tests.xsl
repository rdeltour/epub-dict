...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="html">
    <x:description stylesheet="../schema/schema-{$schema}-compiled.xsl">
  ...
  <xsl:template match="body/*[@class=('valid','invalid')]">
    <x:scenario label="{@title}">
      <x:context href="resources/test-{$schema}-{generate-id()}.xhtml"/>
      <x:expect label="should be {@class}"
        test="{if (@class='valid') then 'empty' else 'exists'}(//svrl:failed-assert)"/>
    </x:scenario>
    <xsl:result-document href="resources/test-{$schema}-{generate-id()}.xhtml">
